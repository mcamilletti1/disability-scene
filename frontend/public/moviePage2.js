const showMoviePage = async () => {
    ...
    const movieId2 = allMovies[2].id
    const response1 = await axios.get(`https://disability-scene-api-production.up.railway.app/movies/${movieId2}/cast`)
    ...
    const response2 = await axios.get(`https://disability-scene-api-production.up.railway.app/movies/${movieId2}/reviews`)
    ...
    const getSceneScore = async () => {
        ...
        for (let i=0; i < reviewsLength; i++)  {
            ...
            AccuracyScore += allReviews[i].accuracy
        }
