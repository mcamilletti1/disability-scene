...
const createReview = async (e) => {
    e.preventDefault()
    const response2 = await axios.get('disability-scene-api-production.up.railway.app/movies')
    const allMovies = response2.data
    const moviesLength = response2.data.length
    const movieId = allMovies[2].id
    let castingInput = 0
    ...
    await axios.post(`disability-scene-api-production.up.railway.app/reviews`, {
            movie: movieId,
