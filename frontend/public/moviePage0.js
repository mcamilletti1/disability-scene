const showMoviePage = async () => {
    ...
    const movieId0 = allMovies[0].id
    const response1 = await axios.get(`https://disability-scene-api-production.up.railway.app/movies/${movieId0}/cast`)
    ...
    const response2 = await axios.get(`https://disability-scene-api-production.up.railway.app/movies/${movieId0}/reviews`)
    ...
    movieTitle.innerHTML = `${allMovies[0].title} (${allMovies[0].year})`
    movieDateGenreDuration = document.querySelector('#dateGenreDuration')
    movieDateGenreDuration.innerHTML = `${allMovies[0].year} | ${allMovies[0].genre} | ${allMovies[0].duration} mins`
    movieDescription = document.querySelector('#movieDescription')
    movieDescription.innerHTML = `${allMovies[0].description}`
    movieImage = document.querySelector('#column1')
    movieImage.innerHTML = `<img id="moviePoster" src="${allMovies[0].img}" width="203px" height="300px" alt="Movie Poster"/>`
    const disabilities = document.querySelector('#disabilities')
    disabilities.innerHTML = `${allMovies[0].disabilities.toString()}`
    const themes = document.querySelector('#themes')
    themes.innerHTML = `${allMovies[0].themes.toString()}`
    const genre = document.querySelector('#genre')
    genre.innerHTML = `${allMovies[0].genre.toString()}`
