const showMoviePage = async () => {
    const response = await axios.get('disability-scene-api-production.up.railway.app/movies')
    const allMovies = response.data
    const movieId1 = allMovies[1].id
    const response1 = await axios.get(`https://disability-scene-api-production.up.railway.app/movies/${movieId1}/cast`)
    ...
    const response2 = await axios.get(`https://disability-scene-api-production.up.railway.app/reviews/${movieId1}/reviews`)
    ...
    movieTitle.innerHTML = `${allMovies[1].title} (${allMovies[1].year})`
    movieDateGenreDuration = document.querySelector('#dateGenreDuration')
    movieDateGenreDuration.innerHTML = `${allMovies[1].year} | ${allMovies[1].genre} | ${allMovies[1].duration} mins`
    movieDescription = document.querySelector('#movieDescription')
    movieDescription.innerHTML = `${allMovies[1].description}`
    movieImage = document.querySelector('#column1')
    movieImage.innerHTML = `<img id="moviePoster" src="${allMovies[1].img}" width="203px" height="300px" alt="Movie Poster"/>`
    const disabilities = document.querySelector('#disabilities')
    disabilities.innerHTML = `${allMovies[1].disabilities.toString()}`
    const themes = document.querySelector('#themes')
    themes.innerHTML = `${allMovies[1].themes.toString()}`
    const genre = document.querySelector('#genre')
    genre.innerHTML = `${allMovies[1].genre.toString()}`
